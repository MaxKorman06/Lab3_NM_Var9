...
namespace Lab3_NM_Var9
{
    public partial class Form1 : Form
    {
        ...
        {
            double x;
            x = a_h - (first_function(a_h) * (b_h - a_h) / (first_function(b_h) - first_function(a_h)));//((2 * Math.Pow(a_h, 2) * b_h - 2 * Math.Pow(a_h, 3) - 4 * a_h * b_h + 4 * Math.Pow(a_h, 2) + 2 * b_h - 2 * a_h - 3 * b_h * Math.Exp(a_h) + 3 * a_h * Math.Exp(a_h)) /
        ...
        public double first_function(double a_h)
        {
            ...
            x = Math.Pow(a_h + 1, 1 / 3) - Math.Exp(-a_h);
        ...
        {
            double x;
            x = (1 / (3 * Math.Pow(Math.Pow(a_h + 1, 2), 1 / 3))) + (1 / Math.Exp(a_h));
            return x;
        }
        ...
        public double second_derivative(double a_h)
        {
            ...
            x = (2 / (9 * (a_h + 1) * Math.Pow(Math.Pow(a_h + 1, 2), 1 / 3))) - (1 / Math.Exp(a_h));
        ...
        {
            double x;
            x = b_h - (first_function(b_h) / first_derivative(b_h));
            return x;
        }
